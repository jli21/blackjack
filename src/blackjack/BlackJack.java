package blackjack;

import java.util.*;

public class BlackJack {

    private final String[] SUITS = { "C", "D", "H", "S" };
    private final String[] RANKS = { "A", "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K" };

    private final Player player;
    private final Player dealer;
    private List<Card> deck;
    private final Scanner in;
    private boolean ongoingTurn = true;
    private int playerScore = 0;
    private int dealerScore = 0;
    private int chips = 100;
    private int wager;
    private boolean turn = true;

    public BlackJack() {
        this.player = new Player();
        this.dealer = new Player();
        this.in = new Scanner(System.in);
    }

    public void play() {
        turn = true;
        wager();
        clearHand("player");
        clearHand("computer");
        shuffleAndDeal();
        takeTurn(false);
        endRound();
    }

    private void wager() {
        do {
            System.out.println("Your total chips right now: " + chips);
            System.out.println("How many chips would you like to wager(Min: 1, Max: 25)");
            wager = in.nextInt();
            if (wager > chips) {
                System.out.println("Sorry, you only have " + chips + " chips remaining, but you wagered " + wager + " chips.");
            }
        } while (wager < 1 || wager > 25 || wager > chips);
        in.nextLine();
    }

    public void shuffleAndDeal() {
        if (deck == null) {
            initializeDeck();
        }
        Collections.shuffle(deck); 
        while (player.getHand(false).size() < 2) {
            player.takeCard(deck.remove(0));   
            dealer.takeCard(deck.remove(0)); 
        }

        playerScore = player.calculateScore();
        dealerScore = dealer.calculateScore();
    }

     
    private void initializeDeck() {
        deck = new ArrayList<>(52);

        for (String suit : SUITS) {
            for (String rank : RANKS) {
                deck.add(new Card(rank, suit));     
            }
        }
    }

    private void takeTurn(boolean cpu) {
        if (!cpu) {
            showHand("initial");
            String action = "x";
            while (action.equals("H") == false && action.equals("S") == false) {
                System.out.println("Hit or Stand(H or S)");
                action = in.nextLine().trim().toUpperCase();
            }
            if (action.equals("H")) {
                player.takeCard(deck.remove(0));
                playerScore = player.calculateScore();
                if (playerScore > 21) {
                    endRound();
                } else {
                    takeTurn(false);
                }

            } else if (action.equals("S")) {
                takeTurn(true);
            }
        } else if (cpu) {
            showHand("cpu");
            while (dealerScore < 17) {
                dealer.takeCard(deck.remove(0));
                dealerScore = dealer.calculateScore();
                showHand("cpu");
            }
        }
    }

    private void endRound() {
        showHand("player");
        showHand("computer");

        if (playerScore > 21) {
            System.out.println("You busted and have lost this round.");
            chips -= (wager);
            if (chips <= 0) {
                endGame();
            }
        } else if(dealerScore > 21) {
            System.out.println("The dealer busted and you have won this round.");
            chips += wager;
        } else if (dealerScore == playerScore && playerScore <= 21) {
            System.out.println("You have tied with the dealer this round.");
        } else if (playerScore == 21 && player.hand.size() == 2) {
            System.out.println("Congratulations, that is a BlackJack. You have won this round.");
            chips += (int) (wager * 1.5);
        } else if (dealerScore > playerScore && dealerScore <= 21) {
            System.out.println("You have lost this round " + playerScore + " to " + dealerScore + ".");
            chips -= wager;
        } else if (playerScore > dealerScore && playerScore <= 21) {
            System.out.println("You have won this round " + playerScore + " to " + dealerScore + ".");
            chips += wager;
        }
        if (chips <= 0) {
            endGame();
        }
        String answer = "";
        do {
            System.out.println("Would you like to end the game here or continue playing(E or C)");
            answer = in.nextLine().toUpperCase();
        } while (answer.equals("E") == false && answer.equals("C") == false);
        if (answer.equals("E")) {
            endGame();
        } else {
            play();
        }

    }

    private void showHand(String type) {
        if (type.equals("initial")) {
            System.out.println("\nPLAYER hand: " + player.getHand(false));   
            System.out.println("\nCPU hand: " + dealer.getHand(true));
        } else if (type.equals("cpu")) {
            System.out.println("\nCPU hand: " + dealer.getHand(false)); 
        } else if (type.equals("player")) {
            System.out.println("\nPLAYER hand: " + player.getHand(false));
        }
    }

    private void clearHand(String type) {
        if (type.equals("player")) {
            player.clearHand();
        } else if (type.equals("computer")) {
            dealer.clearHand();
        }
    }

    private void endGame() {
        if (chips <= 0) {
            System.out.println("You ran out of chips and lost.");
        } else {
            System.out.println("You ended up with " + chips + " chips.");
        }
        System.exit(0);
    }

    public static void main(String[] args) {
        System.out.println("BLACKJACK");
        new BlackJack().play();
    }
}